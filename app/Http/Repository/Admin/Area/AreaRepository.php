...
class AreaRepository
{
    ...
    public function getTalukaList() {
        $data_taluka = TblArea::where('tbl_area.location_type','=','3')
                        ->where('tbl_area.is_active','=','1')
						->select('tbl_area.location_id',
								'tbl_area.name',
                                'tbl_area.is_active'
							)
		...
		public function deleteTaluka($id)
{
    try {
        ...
        $taluka = TblArea::find($id);
        if ($taluka) {
            // Set the is_delete, is_active, and is_new flags for the village
            $taluka->is_active = '0'; // Assuming you want to deactivate it
            $taluka->is_active = '0';
            $taluka->is_new = '1';    // Assuming you want to mark it as not new
            $taluka->save();
	...
	public function showVillage($id)
	{
		try {
			...
			$data_users = [];
			$user = TblArea::leftJoin('tbl_area as taluka_data', 'tbl_area.parent_id', '=', 'taluka_data.location_id')
						->where('tbl_area.location_type','=','4')
						->where('tbl_area.location_id', $id)
						->select('tbl_area.location_id',
								'tbl_area.name',
								'tbl_area.is_active',
								'taluka_data.name as taluka_name'
			...
			$data_users['user'] = $user[0];				

			$data_users['ward_data'] = Wards::where('wards.village_id', $id)
						->select('wards.ward_name'
							)
							->orderByRaw('CAST(wards.ward_name AS UNSIGNED) ASC')
							->get();				
    ...
    public function addVillageInsert($request)
	{
		...
		$area_data->is_new = '1';
		$area_data->is_active = isset($request['is_active']) ? true : false;
		$area_data->is_ward = isset($request['is_ward']) ? true : false;
		$area_data->save();
		$last_insert_id = $area_data->location_id;
if(isset($request['ward_no_id'])){
		foreach ($request['ward_no'] as $key => $data) {
			// dd($data);
			$ward_data = new Wards();
		$ward_data->village_id = $last_insert_id;
		$ward_data->ward_name = $data;
		$ward_data->save();
		}
}
    ...
    public function editVillage($reuest)
	{
		...
		$data_users_data = TblArea::leftJoin('tbl_area as taluka_data', 'tbl_area.parent_id', '=', 'taluka_data.location_id')
			...
			->select(
				...
				'tbl_area.is_active',
				'tbl_area.is_ward',
				'taluka_data.location_id as taluka_id'
			)->get()
			->toArray();

		$data_users['data_village'] = $data_users_data[0];

		$data_wards_data = Wards::where('wards.village_id', '=', base64_decode($reuest->edit_id))
							->where('wards.is_deleted', '=', '0')
			->select(
				'wards.id',
				'wards.ward_name'
			)->get()
			...
			$data_users['data_wards'] = $data_wards_data;
	// dd($data_village);
    ...
    public function updateVillage($request)
	{
		...
		$user_data = TblArea::where('location_id',$request['edit_id']) 
					->update([
						...
						'is_new' => '1',
						'is_active' => isset($request['is_active']) ? true :false,
						'is_ward' => isset($request['is_ward']) ? true :false,
					]);

                if(isset($request['ward_no_id'])){
				foreach ($request['ward_no'] as $key => $data) {
					if($request['ward_no_id'][$key] !=''){
					Wards::where('id',$request['ward_no_id'][$key]) 
					->update([
						'ward_name' => $data,
					]);
				}elseif($request['ward_no_id'][$key] !='null')
				{
					// dd($request['edit_id']);
					$ward_data = new Wards();
					$ward_data->village_id = $request['edit_id'];
					$ward_data->ward_name = $data;
					$ward_data->save();
				}
				}
	...
	public function deleteVillage($id)
{
    try {
        // Find the village by ID
        $village = TblArea::find($id);
        if ($village) {
            // Set the is_delete, is_active, and is_new flags for the village
            $village->is_active = '0'; // Assuming you want to deactivate it
            $village->is_new = '1';    // Assuming you want to mark it as not new
            $village->save();

            // Set the is_delete, is_active, and is_new flags for all associated wards
            Wards::where('village_id', $id)->update([
                'is_deleted' => '1',
                'is_active' => '0', // Deactivate the ward as well
                'is_new' => '1'     // Mark the ward as not new
            ]);

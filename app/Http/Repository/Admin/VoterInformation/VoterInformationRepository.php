<?php
namespace App\Http\Repository\Admin\VoterInformation;
...
class VoterInformationRepository
{

	public function index() {

		$data_users = VoterInformation::leftJoin('tbl_area as taluka_user', 'voter_information.taluka_id', '=', 'taluka_user.location_id')
				...
				'taluka_user.name as taluka','village_user.name as village','voter_information.mobile_number')
				->get();
// dd($data_users);
	...
	public function showVoterInformation($id)
	{
		...
		try {
			...
			$data_gram_doc['user_data'] = VoterInformation::leftJoin('tbl_area as taluka_user', 'voter_information.taluka_id', '=', 'taluka_user.location_id')
				->leftJoin('tbl_area as village_user', 'voter_information.village_id', '=', 'village_user.location_id')
				->leftJoin('users', 'voter_information.added_by', '=', 'users.id')
				->leftJoin('casts', 'voter_information.cast', '=', 'casts.id')
				->leftJoin('gender', 'voter_information.gender', '=', 'gender.id')
				->where('voter_information.id',$id)
				->where('voter_information.is_active','1')
				->select('voter_information.f_name as voter_f_name','voter_information.m_name as voter_m_name','voter_information.l_name as voter_l_name',
				'voter_information.id as voter_info_id','users.id','users.f_name',
				'users.m_name','users.l_name','taluka_user.name as taluka',
				'village_user.name as village','voter_information.mobile_number',
				'voter_information.voter_id','voter_information.ward_no','voter_information.date_of_birth',
				'casts.cast_name','gender.gender_name','voter_information.other_cast_name')
				->first();
